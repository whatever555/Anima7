...
public class Canvas extends PApplet {
	...
	public void draw() {
		...
		if (parent.currentTool.equals("brush") && drawBool) {
			...
			inks.add(new Ink(pmouseX, pmouseY, mouseX, mouseY, parent.PENSIZE, localPenColor, parent.PENALPHA));

			mylineBasic(pmouseX, pmouseY, mouseX, mouseY);
		} else if (parent.currentTool.equals("Eraser") && drawBool) {
			mylineBasic(pmouseX, pmouseY, mouseX, mouseY);

			eraseThatShizzle();
		} else if (parent.currentTool.equals("move")) {
			...
			if (drawBool) {
				...
				if (resizing) {
					if (resizeDirection.indexOf("E") > -1)
						clipBoardWidth += mouseX - (clipBoardX + clipBoardWidth);
					if (resizeDirection.indexOf("W") > -1) {
						clipBoardWidth -= (mouseX - clipBoardX);
						clipBoardX = mouseX;
					}
					if (resizeDirection.indexOf("S") > -1)
						clipBoardHeight += mouseY - (clipBoardY + clipBoardHeight);
			...
			if (clipBoardWidth < 0 && clipBoardHeight < 0) {
				...
				image(floatingClipBoard, -clipBoardX, -clipBoardY, -clipBoardWidth, -clipBoardHeight);
				popMatrix();
		...
		public void runSaveFile() {
			new Thread() {
				...
				public void run() {
					...
					String sp = savePath(
							parent.workspaceFolder + "/data/" + parent.tmpName + "/" + cl + "_" + ck + ".png");
					img.save(sp);
					SAVING = false;
					img = null;
	...
	public void mouseDragged() {
		...
		if (parent.currentTool.equals("star") || parent.currentTool.equals("circle")
				|| parent.currentTool.equals("rect")) {
			jutra++;
			if (jutra % 5 == 0) {
				jutra = 1;
				tempGraphic.clear();
				...
				if (parent.currentTool.equals("rect")) {
					...
					tempGraphic.rect(min(selectBeginX, mouseX), min(selectBeginY, mouseY),
							...
							max(selectBeginY, mouseY) - min(selectBeginY, mouseY), parent.SHAPEROUNDCORNERSIZE);
				} else if (parent.currentTool.equals("circle")) {
			...
			if (jutra % 5 == 0) {
				...
				if (parent.currentTool.equals("selectRect")) {
					...
					tempGraphic.rect(min(selectBeginX, mouseX), min(selectBeginY, mouseY),
							max(selectBeginX, mouseX) - min(selectBeginX, mouseX),
							max(selectBeginY, mouseY) - min(selectBeginY, mouseY), parent.ROUNDCORNERSIZE);
				}
	...
	public void exportImages(final boolean createAnimatedGIF, final boolean createSWF, final String STRextension,
			final String location) {

		boolean trans = false;
		int selectedOption = JOptionPane.showConfirmDialog(null, translate("Make background transparent?"),
		...
		Thread t = new Thread() {
			public void run() {
				...
				if (createSWF) {
					outSWF = new SWFExport(cw, ch, parent.FPS, null, false, location + "/swf.swf");
				...
				for (int x = 0; x <= parent.lastFrame; x++) {
						...
						if (parent.timeline.layers.get(y).visible)
							if (!parent.timeline.layers.get(y).isMask) {
								...
								if (parent.timeline.layers.get(y).hasMask) {
									int id = parent.timeline.layers
											.get(getLayerIndex(parent.timeline.layers.get(y).myMask)).layerID;
					...
					if (!createAnimatedGIF) {
						String sp = savePath(location + "/" + x + "." + STRextension);
						tmp.save(sp);
					}
					if (createAnimatedGIF)
					...
					if (createSWF) {
						...
						outSWF.addImage(location + "/" + x + "." + STRextension);
	 ...
	 * 
	 * int imgx = min(clipBoardX, clipBoardX + clipBoardWidth); int imgxw =
	 * max(clipBoardX, clipBoardX + clipBoardWidth); int imgy = min(clipBoardY,
	 * clipBoardY + clipBoardHeight); int imgyh = max(clipBoardY, clipBoardY +
	 * clipBoardHeight);
	 * 
	 * boolean north = false, west = false, south = false, east = false;
	 * 
	 * if (clipBoardHeight > 0) { north = (y < imgy && y > imgy - 10); south =
	 * (y < imgyh +10 && y > imgyh); } else { south = (y < imgy && y > imgy
	 * -10); north = (y < imgyh +10 && y > imgyh); }
	 * 
	 * if (clipBoardWidth > 0) {
	 * 
	 * west = (x > imgx -10 && x < imgx); east = (x < imgxw +10 && x > imgxw );
	 * } else { east = (x > imgx -10 && x < imgx ); west = (x < imgxw +10 && x >
	 * imgxw );
	 * 
	 * } rotating = true; moving = false; if (north && east) { rotateDirection =
	 * "NE"; parent.setCursor("rotate"); } else if (north && west) {
	 * rotateDirection = "NW"; parent.setCursor("rotate"); } else if (south &&
	 * east) { rotateDirection = "SE"; parent.setCursor("rotate"); } else if
	 * (south && west) { rotateDirection = "SW"; parent.setCursor("rotate"); }
	 * else if (east) { skewDirection = "E"; parent.setCursor("skew"); } else if
	 * (west) { skewDirection = "W"; parent.setCursor("skew"); } else if (north)
	 * { skewDirection = "N"; parent.setCursor("skew"); } else if (south) {
	 * skewDirection = "S"; parent.setCursor("skew"); } else if (rotating) {
	 * rotating = false; }
	 * 
	 * }
	 */
	...
	public void mousePressed() {
		...
		if (parent.currentTool.equals("selectRect") || parent.currentTool.equals("selectCirc")
				|| parent.currentTool.equals("selectShape")) {
		...
		} else if (parent.currentTool.equals("star") || parent.currentTool.equals("circle")
				|| parent.currentTool.equals("rect")) {
			tempGraphic = createGraphics(viewWidth, viewHeight);
			tempGraphic.beginDraw();
			...
			if (!isImage(parent.CURRENTLAYER, parent.CURRENTFRAME))
				saveAction(parent.CURRENTLAYER, parent.CURRENTFRAME, "Empty Canvas");
		}
	...
	public void addAudioToKeyFrame(int cl, int cf, String audioPath) {
		...
		// parent.timeline.layers.get(getLayerIndex(cl)).jbs.get(getKeyFrame(cf,
		// getLayerIndex(cl))).hasAudio=true;
		// parent.timeline.layers.get(getLayerIndex(cl)).jbs.get(getKeyFrame(cf,
		// getLayerIndex(cl))).audioFile=audioPath;
	}
	...
	public void removeAudioFromKeyFrame(int cl, int cf) {
		...
		// parent.timeline.layers.get(getLayerIndex(cl)).jbs.get(getKeyFrame(cf,
		// getLayerIndex(cl))).hasAudio=false;
		// parent.timeline.layers.get(getLayerIndex(cl)).jbs.get(getKeyFrame(cf,
		// getLayerIndex(cl))).audioFile="";
	}
	...
	public void mouseReleased() {
		...
		if (parent.currentTool.equals("rect") || parent.currentTool.equals("circle")
				|| parent.currentTool.equals("star")) {
			unsaved = true;
			keyEdited = true;
			currentFrameGraphic.beginDraw();
		...
		if (parent.currentTool.equals("brush")) {
			...
			new Thread() {
				public void run() {
					setSave(currentlySaving, false, parent.CURRENTLAYER, parent.CURRENTFRAME, "Paint");
		...
		if (parent.currentTool.equals("Eraser")) {
			...
			new Thread() {
				public void run() {
					setSave(currentlySaving, false, parent.CURRENTLAYER, parent.CURRENTFRAME, "Erase");
	...
	boolean justDoneADo = false; // set to true when either undo or redo is
									// preformed
	...
	public void saveAction(int cl, int cf, String label) {
		...
		if (!saving) {
			...
			saving = true;
			unsaved = false;
			if (!label.equals("Keyframe Removed") && !label.equals("Keyframe Added")) {
			...
			if (!label.equals("Keyframe Added") && !label.equals("KEYADDFIX") && !label.equals("KEYREMOVEFIX")
					&& !label.equals("Keyframe Removed") && (lastSavedLayer != cl || lastSavedFrame != ck)) {
	...
	public void mylineBasic(int x, int y, int x2, int y2) {
		...
		for (int i = 0; i <= longest && brush.size() > 0; i += max(1, abs(parent.PENSIZE / 14))) {
	...
	public void layDownFrames(int hiddenLayer) {
		...
		if (AUDIOLAYER > -1)
			if (parent.timeline.layers.get((AUDIOLAYER)).jbs.get(parent.CURRENTFRAME).isKey) {
				stopAudio();
				if (parent.timeline.layers.get(AUDIOLAYER).jbs.get(parent.CURRENTFRAME).hasAudio
						&& parent.playPreviewBool) {
		...
		for (int i = parent.MAXLAYERS - 1; i > -1; i--) {
			...
			if (parent.timeline.layers.get(i).visible && !parent.timeline.layers.get(i).activeMask
					&& parent.timeline.layers.get(i).layerID != parent.CURRENTLAYER && i != hiddenLayer) {
				...
				if (tmp != null) {
					...
					if (parent.timeline.layers.get(i).hasMask) {
						if (parent.timeline.layers
								.get(parent.canvas.getLayerIndex(parent.timeline.layers.get(i).myMask)).activeMask) {

							PImage maskImage = loadImageFromDisk(
									parent.timeline.layers.get(
											parent.canvas.getLayerIndex(parent.timeline.layers.get(i).myMask)).layerID,
									parent.CURRENTFRAME);
							// tmp.mask(maskImage);
		...
		for (int i = Math.max(0, parent.CURRENTFRAME - parent.onionLeft); i <= Math.min(parent.MAXFRAMES - 1,
				parent.CURRENTFRAME + parent.onionRight); i++) {
			if (parent.timeline.layers.get(getLayerIndex(parent.CURRENTLAYER)).visible)
				if (getKeyFrame(i, getLayerIndex(parent.CURRENTLAYER)) != getKeyFrame(parent.CURRENTFRAME,
						getLayerIndex(parent.CURRENTLAYER)))
					if (!shownKeys.contains(getKeyFrame(i, getLayerIndex(parent.CURRENTLAYER)))
							&& parent.CURRENTLAYER != hiddenLayer) {
						setBlending(tmpG, parent.CURRENTLAYER);
						setBlending(currentFrameGraphic, parent.CURRENTLAYER);
						float fx = (float) ((float) ((float) onionTransValue / 255))
								* parent.timeline.layers.get(parent.CURRENTLAYER).alphaLevel;
						tmpG.tint(255, fx);
						...
						if (parent.timeline.layers.get(getLayerIndex(parent.CURRENTLAYER)).hasMask) {
							PImage maskImage = loadImageFromDisk(parent.timeline.layers.get(getLayerIndex(
									parent.timeline.layers.get(getLayerIndex(parent.CURRENTLAYER)).myMask)).layerID, i);
							// tmp.mask(maskImage);
							tmp = drawMaskedAdvanced(tmp, maskImage);
		...
		int maxTriesCount = 0;
		if (parent.timeline.layers.get(getLayerIndex(parent.CURRENTLAYER)).visible
				&& hiddenLayer != parent.CURRENTLAYER) {
			while (tmp == null && maxTriesCount < 4) {
				...
				if (tmp != null) {
					...
					if (parent.CURRENTLAYER != hiddenLayer
							&& !parent.timeline.layers.get(getLayerIndex(parent.CURRENTLAYER)).activeMask)
						if (parent.timeline.layers.get(getLayerIndex(parent.CURRENTLAYER)).hasMask) {
							PImage maskImage = loadImageFromDisk(
									parent.timeline.layers.get(parent.canvas.getLayerIndex(parent.timeline.layers
											.get(getLayerIndex(parent.CURRENTLAYER)).myMask)).layerID,
									parent.CURRENTFRAME);
							// tmp.mask(maskImage);
							tmp = drawMaskedAdvanced(tmp, maskImage);
	...
	PImage drawMaskedAdvanced(PImage img, PImage mask) {
		...
		for (int i = 0; i < img.pixels.length; i++) {
			if ((alpha(img.pixels[i]) > 0) && (alpha(mask.pixels[i]) > 0)) {
				tmp.pixels[i] = color(red(img.pixels[i]), green(img.pixels[i]), blue(img.pixels[i]),
						(alpha(mask.pixels[i]) / 255) * alpha(img.pixels[i]));
			} else {
	...
	PImage eraseThisFromThat(PImage mask, PImage img, int x, int y, int px, int py) {
		...
		for (int p = startX; p < endX; p++)
			for (int j = startY; j < endY; j++) {
				...
				if ((alpha(img.pixels[i]) > 0) && (alpha(mask.pixels[i]) > 0)) {
					tmp.pixels[i] = color(red(img.pixels[i]), green(img.pixels[i]), blue(img.pixels[i]),
							(alpha(img.pixels[i])) - alpha(mask.pixels[i]));
				}
			}
		return tmp;
	}
	...
	public PImage loadImageFromDisk(int cl, int cf) {
		...
		if (parent.cachedImagesNames.contains("image" + cl + "_" + ck)) {
			...
			return loadImagex(parent.workspaceFolder + "/data/" + parent.tmpName + "/" + cl + "_" + ck + ".png");
	...
	public PImage loadImageFromCache(int l, int k) {
		return parent.cachedImages.get(parent.cachedImagesNames.indexOf("image" + l + "_" + k));
	}
	...
	public void deleteImageFromDisk(int cl, int ck) {
		if (parent.cachedImagesNames.contains("" + cl + "_" + ck)) {
			parent.cachedImages.remove(parent.cachedImagesNames.indexOf("image" + cl + "_" + ck));
		}
	...
	public void addToCache(PImage img, int cl, int ck) {
		...
		if (!((parent.cachedImagesNames.contains("image" + cl + "_" + ck)))) {
			...
			parent.cachedImages.set(parent.cachedImagesNames.indexOf("image" + cl + "_" + ck), img);
	...
	public boolean finaliseFrameBool(int layer, int frame) {
		...
		if (keyEdited) {
			...
			tempImg = null;
			// tempImg.resize(parent.timelineButtonWidth,
			// parent.timelineButtonHeight);
			// BufferedImage tmp = (BufferedImage) (tempImg.getNative());
			// parent.timeline.layers.get(getLayerIndex(layer)).jbs.get(lk).setIcon(new
			// ImageIcon(tmp));
	...
	public void pasteFromClipBoard(boolean clearClipBoard, String saveMessage) {
		...
		if (clipBoard != null) {
			...
			parent.pasting = true;
			if (mouseX > 0 && mouseX < width - 1 && mouseY > 0 && mouseY < height - 1 && !clearClipBoard) {
	...
	public void finalisePaste() {
		...
		if (clipBoardWidth < 0 && clipBoardHeight < 0) {
			...
			currentFrameGraphic.image(floatingClipBoard, -clipBoardX, -clipBoardY, -clipBoardWidth, -clipBoardHeight);
	...
	public void fillGraphic(int x, int y) {
		...
		int newCol = color(parent.PENCOLOR.getRed(), parent.PENCOLOR.getGreen(), parent.PENCOLOR.getBlue(),
	...
	public PGraphics fillIterative(int oldCol, int newCol, int x, int y, int inaccuracy, boolean fillTool) {
		...
		while (!queue.isEmpty()) {
			...
			if ((p.x >= 0) && (p.x < width && (p.y >= 0) && (p.y < height))) {
				int tmp = (int) alpha(currentFrameGraphic.pixels[p.x + (p.y * width)]);

				if ((tmp > 0 || ignoreAlpha) && (checked[p.x][p.y] != 1) && compareColors(
						currentFrameGraphic.pixels[p.x + (p.y * width)], oldCol, inaccuracy, ignoreAlpha)) {

					if (!ignoreAlpha) {

						pg.pixels[p.x + (p.y * width)] = color(red(newCol), green(newCol), blue(newCol), tmp);

					} else
						pg.pixels[p.x + (p.y * width)] = color(red(newCol), green(newCol), blue(newCol), 255);
	...
	public void defaultFilters(float[] amounts, boolean apply, String filterName) {
		...
		if (!apply) {
			...
			if (filterName.equals("Blur"))
				...
				previewThumb = lensnBlurFilter(previewThumb, (float) (amounts[0] / 10), (float) (amounts[1] / 10),
						(float) (amounts[2] / 360), (float) (amounts[3]));
				...
				previewThumb = ShadowFilter(previewThumb, (int) (amounts[0]), (int) (amounts[1]), (int) (amounts[2]),
						(float) (amounts[3]));
			...
			if (filterName.equals("Blur"))
				...
				tmp = ShadowFilter(tmp, (int) (amounts[0]), (int) (amounts[1]), (int) (amounts[2]),
						(float) (amounts[3]));
			else if (filterName.equals("Glow"))
				...
				tmp = lensnBlurFilter(tmp, (float) (amounts[0] / 10), (float) (amounts[1] / 10),
						(float) (amounts[2] / 360), (float) (amounts[3]));
			else if (filterName.equals("Threshold"))
			...
			new Thread() {
				public void run() {
					setSave(currentlySaving, false, parent.CURRENTLAYER, parent.CURRENTFRAME, "Filter");
	...
	public void drawCurve(PGraphics pg, int bristle) {
		...
		for (int i = 0; i < cv.length - (bristle); i++) {
			pg.curveVertex(cv[i][0], cv[i][1]); // is also the start point of
												// curve
